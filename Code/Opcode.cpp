...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[ADDI].name = "addi";  myArray[ADDI].numOps = 3;   myArray[ADDI].rdPos = -1;  myArray[ADDI].rsPos = 0;  myArray[ADDI].rtPos = 1;   myArray[ADDI].immPos = 2;  myArray[ADDI].instType = ITYPE; myArray[ADDI].op_field = "001000"; myArray[ADDI].funct_field = ""; 
  ...
  myArray[LB].name = "lb";  myArray[LB].numOps = 3;   myArray[LB].rdPos = -1;  myArray[LB].rsPos = 0;  myArray[LB].rtPos = 2;   myArray[LB].immPos = 1;  myArray[LB].instType = ITYPE; myArray[LB].op_field = "100000"; myArray[LB].funct_field = "";
  ...
  myArray[BEQ].name = "beq";  myArray[BEQ].numOps = 3;   myArray[BEQ].rdPos = -1;  myArray[BEQ].rsPos = 1;  myArray[BEQ].rtPos = 1;   myArray[BEQ].immPos = 2;  myArray[BEQ].instType = ITYPE; myArray[BEQ].op_field = "000100"; myArray[BEQ].funct_field = ""; myArray[BEQ].immLabel = true; myArray[BEQ].isAddress = true;
...
//Like getOpcode, but accounts for a function field
Opcode OpcodeTable::getOpcode(string str, string funct)
{
  for(int i = 0; i < (int)UNDEFINED; i++){
    if(myArray[i].op_field == str && myArray[i].funct_field == funct){
      return (Opcode)i;
    }
  }
  for(int i = 0; i < (int)UNDEFINED; i++){ //if it's not an RTYPE, find normal opcode
    if(myArray[i].op_field == str){
      return (Opcode)i;
    }
  }
...
// Necessary for the "beq" instruction; returns true if instruction has address in imm field
